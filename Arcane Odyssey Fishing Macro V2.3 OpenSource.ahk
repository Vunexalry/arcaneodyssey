...
if remainingSeconds > 0 {
    ...
    FileAppend("Trial time was expired or invalid. Trial remains expired until new download is detected.`n", A_ScriptDir "\Debug.log")
}
...
;Removed for open source to prevent unauthorized premium bypass, tampering, or code 
;duplication. This encrypted strings with a key, for securing data like 
;trial times in storage.
    return encrypted
}
...
SaveBackupToAreaFiles(backupValue, backupType) {
    global fish_caught_x1, fish_caught_y1, fish_caught_x2, fish_caught_y2
    global bait_x1, bait_y1, bait_x2, bait_y2
    
    ;Removed for open source to prevent unauthorized premium bypass, tampering, or code 
    ;duplication. This function encrypted and saved trial time as pipe-delimited backup data in  
    ;files, ensuring persistent storage to prevent corruption of data.
    return
}
...
GetAOVunoxPremiumSerial() {
    ...
    ;the HWID and converting it to an x-character alphabetic string for license verification. return ""
}
...
ValidateOTP(otp) {
    ...
    ;the HWID, then activating premium by setting registry 
...
MyWindow.AddHostObjectToScript("MacroFuncs", {
    ...
    updateExitKey: UpdateExitKey,
    getCameraAdjustKey: (*) => ReverseKeyFormat(cameraAdjustKey),
    getStartMacroKey: (*) => ReverseKeyFormat(startMacroKey),
    getReloadMacroKey: (*) => ReverseKeyFormat(reloadMacroKey),
    getScreenshotKey: (*) => ReverseKeyFormat(screenshotKey),
    getExitKey: (*) => ReverseKeyFormat(exitKey)
})
...
global fish_caught_confidence := 60
...
fish_caught_confidence := IniRead(A_ScriptDir "\Settings.ini", "Confidence", "fish_caught_confidence", 60)
...
CheckForFish() {
    global fishDetected, fish_caught_x1, fish_caught_y1, fish_caught_x2, fish_caught_y2
    if IsTextInArea("caught", fish_caught_x1, fish_caught_y1, fish_caught_x2, fish_caught_y2) {
        fishDetected := true
    }
}
...
SelectBait(baitType) {
    ...
    local x1 := bait_x1, y1 := bait_y1, x2 := bait_x2, y2 := bait_y2
    local w := x2 - x1, h := y2 - y1
    
    static failMagicCount := 0
    if (baitType = "magic") {
        local foundNormal := ""
        Loop 2 {
            try {
                local scale := (A_Index == 1) ? 1 : 2
                local result := OCR.FromRect(x1, y1, w, h, {scale: scale})
                foundNormal := result.FindString("normal")
                if foundNormal
                    break
            } catch as e {
                WriteDebug("OCR failed in SelectBait (magic/normal): " e.Message ". Retrying with scale " (A_Index == 1 ? 2 : 1))
                try {
                    local scale := (A_Index == 1) ? 2 : 1
                    local result := OCR.FromRect(x1, y1, w, h, {scale: scale})
                    foundNormal := result.FindString("normal")
                    if foundNormal
                        break
                } catch as e2 {
                    WriteDebug("OCR retry failed: " e2.Message)
                    Sleep(50)
                }
            }
            Sleep(50)
        ...
        if !foundNormal {
            failMagicCount++
            if (failMagicCount >= 5) {
                WriteDebug("OCR completely failed for 'normal' bait in SelectBait after " failMagicCount " consecutive failures.")
            }
            MyWindow.ExecuteScript("showHostToast('OCR error: Could not find normal bait for magic selection. Check bait area.', 'error');")
            return
        } else {
            failMagicCount := 0
        }
        
        local normalClickX := foundNormal.x + (foundNormal.w / 2)
        local normalClickY := foundNormal.y + (foundNormal.h / 2)
    ...
    Loop 2 {
        try {
            ...
            WriteDebug("OCR failed in SelectBait (" baitText "): " e.Message ". Retrying with scale " (A_Index == 1 ? 2 : 1))
            try {
                local scale := (A_Index == 1) ? 2 : 1
                local result := OCR.FromRect(x1, y1, w, h, {scale: scale})
                found := result.FindString(baitText)
    ...
    if !found {
        failBaitCount++
        if (failBaitCount >= 5) {
            WriteDebug("OCR completely failed for '" baitText "' bait in SelectBait after " failBaitCount " consecutive failures.")
        }
        MyWindow.ExecuteScript("showHostToast('OCR error: Could not find " baitText " bait. Check bait area or type.', 'error');")
        return
    } else {
        failBaitCount := 0
    }
    
    local clickX := found.x + (found.w / 2)
    local clickY := found.y + (found.h / 2)
    Sleep(150)
...
CreateScreenshotRect() {
    ...
    ToolTip("Drag the rectangle to position the screenshot area. Use arrow keys to resize. Press " ReverseKeyFormat(screenshotKey) " to capture.")
}
...
IsTextInArea(text, x1, y1, x2, y2) {
    ...
    if (A_TickCount - lastOcrTime < 200) {
        return (cachedResult == "true")
    }
    lastOcrTime := A_TickCount

    if (A_TickCount < cacheExpiry && cachedResult != "") {
        return (cachedResult == "true")
    }
    Loop 2 {
        try {
            local scale := (A_Index == 1) ? 1 : 2
            local result := OCR.FromRect(x1, y1, w, h, {scale: scale})
            local found := result.FindString(text)
            if IsSet(found) {
                failCount := 0
                cachedResult := "true"
                cacheExpiry := A_TickCount + 500
                return true
            }
        } catch as e {
            WriteDebug("OCR failed in IsTextInArea (" text "): " e.Message ". Retrying with scale " (A_Index == 1 ? 2 : 1))
            try {
                local scale := (A_Index == 1) ? 2 : 1
                local result := OCR.FromRect(x1, y1, w, h, {scale: scale})
                local found := result.FindString(text)
    ...
    failCount++
    if (failCount >= 5) {
        WriteDebug("OCR completely failed for text '" text "' in area (" x1 "," y1 "," x2 "," y2 ") after " failCount " consecutive failures.")
        failCount := 0
    }
    cachedResult := "false"
    cacheExpiry := A_TickCount + 500
...
CreateSelectionRect() {
    ...
    OnMessage(0x201, WM_LBUTTONDOWN)
    OnMessage(0x202, WM_LBUTTONUP)
    ToolTip("Drag the rectangle to position the area. Use arrow keys to resize. Press " ReverseKeyFormat(screenshotKey) " to save the " currentArea " area.")
}
...
LoadAllSettings() {
    global current_method, current_key, current_secondary_key, fish_trigger_confidence, fish_caught_confidence, xpos_fish_caught, ypos_fish_caught, xpos_fish_trigger, ypos_fish_trigger, fish_caught_x1, fish_caught_y1, fish_caught_x2, fish_caught_y2, fish_trigger_x1, fish_trigger_y1, fish_trigger_x2, fish_trigger_y2, bait_x1, bait_y1, bait_x2, bait_y2, toggleSecondary, cameraAdjustKey, startMacroKey, reloadMacroKey, screenshotKey, exitKey
    current_method := IniRead(A_ScriptDir "\Settings.ini", "Settings", "current_method", "normal")
    ...
    exitKey := IniRead(A_ScriptDir "\Settings.ini", "Keybinds", "exitKey", "F4")
...
SetDynamicHotkeys() {
    global cameraAdjustKey, startMacroKey, reloadMacroKey, screenshotKey, exitKey
    try {
    ...
    try {
        Hotkey(screenshotKey, HandleScreenshotKey, "On")
    } catch {
        WriteDebug("Error setting screenshot hotkey: " screenshotKey)
    }
    try {
        Hotkey(exitKey, SaveAndExit, "On")
    } catch {
        WriteDebug("Error setting exit hotkey: " exitKey)
    }
}
...
UpdateExitKey(newKey) {
    ...
    if (exitKey != newKey) {
        ...
        try {
            ...
            WriteDebug("Error setting new exit hotkey: " newKey)
            return
        }
    }
    IniWrite(exitKey, A_ScriptDir "\Settings.ini", "Keybinds", "exitKey")
    UpdateSettingsHMAC()
}
